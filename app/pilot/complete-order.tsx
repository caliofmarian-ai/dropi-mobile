...
export default function CompleteMarketplaceOrderScreen() {
  ...
  const validOrderId = useMemo(() => Number.isSafeInteger(orderId) && orderId > 0, [orderId]);
  const transition = trpc.operations.transitionOrder.useMutation();
  ...
  const complete = async () => {
    ...
    try {
      ...
      Alert.alert("Delivery completed", "Proof of delivery was recorded in the operational evidence chain.", [
  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]} className="px-4 pt-4">
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        <Text className="text-2xl font-bold text-foreground">Proof of Delivery</Text>
        <Text className="text-sm text-muted mt-1 mb-5">
          Record what actually happened. Recipient confirmation is separate and optional because DROPi also supports passive terrestrial and drone reception.
        </Text>
        ...
        <TouchableOpacity
          ...
          disabled={!validOrderId || transition.isPending}
          onPress={complete}
        >
          <Text className="text-white font-bold">Record proof & complete delivery</Text>
        </TouchableOpacity>
      </ScrollView>
    </ScreenContainer>
  );
